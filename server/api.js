...
module.exports = Router()
  .get('/comment/:ids', async (req, res) =>
    send(res, await getComments(req.params.ids))
  ) // no cache
  .get('/item/:id', async (req, res) =>
    send(res, await Cache.getKey('item', req.params.id))
  )
  .get('/user/:name', async (req, res) =>
    send(res, await Cache.getKey('user', req.params.name))
  )
  .get('/:type', async (req, res) =>
    send(res, await Cache.getPage(req.params.type, req.query.page))
  );
